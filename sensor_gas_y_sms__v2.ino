...
String Num_Movil[] = {"9211112377","9211978646","9212395682"}; // numeros moviles a quien se envia el mensaje el mensaje, poner el mismo en Cantidad_Movil
...
int valor_limite = 200;                 // Fija el valor limite en el que se activa la alarma
// Fije el valor despues de visualizar el nivel con el Monitor Serial
void setup()
{
  SIM900.begin(19200); //arduino se comunica a esa velocidad
  Serial.begin(19200); //velocidad del puerto serial del arduino
  delay(25000);//tiempo para esperar señal
  pinMode(x,OUTPUT);
// SIM900.print("AT+CMGF=1\r"); comando para poner en modo sms
  delay(200);
  SIM900.print("AT+CNMI=2,2,0,0,0\r");//config modulo para enviar sms por puerto serie
  delay(200);
...
void enviar_SMS() {
  for (int n = 3; n < Cantidad_Movil; n++) {
    delay(1000);
    SIM900.println ("AT + CMGS = \"9211112377, 9211978646,9212395682 \"");  // Envia mensaje
    delay(100);
    SIM900.print(" -- Alerta!! Se detecto la presencia de GAS -- ");    // mensaje que se envía
    delay(100);
